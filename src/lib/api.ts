...
/**
 * GET /api/chat-bot/sessions — Récupère toutes les sessions chatbot actives ou passées.
 */
export async function listChatbotSessions(): Promise<ApiResponse<ChatbotSession[]>> {
  return apiFetch<ChatbotSession[]>('/chat-bot/sessions');
}

/**
 * PUT /api/chat-bot/sessions/:id — Active ou désactive (escalade) une session chatbot.
 */
export async function toggleChatbotSession(id: string, active: boolean): Promise<ApiResponse<{ success: boolean }>> {
  return apiFetch<{ success: boolean }>(`/chat-bot/sessions/${encodeURIComponent(id)}`, {
    method: 'PUT',
